...
class Band {
  ...
  // До приватного поля #members яке є масивом додаємо нового музиканта
  // Якщо ні виводимо в консоль повідомлення Новий учасник повинен бути екземпляром класу Musician
  playMusic() {
    this.#members.forEach((members) => members.play());
  }

  // Створюємо метод playMusic(), за допомогою forEach перебираємо масив і викликаємо метод play()
  // для кожного учасника гурту
}
...
class Performance {
  ...
  // Визначаємо метод info(), що виводить рядок в консоль `Гурт ${this.#band.name} виступить в 
  // ${this.#location} ${this.#date.toLocaleDateString()}`
}
...
const concert = new Concert(band, 'Liverpool', new Date('2023-08-01'), 100);
/*
